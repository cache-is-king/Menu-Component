...
describe('server request handling', () => {
  ...
  test('should return correct data shape for lunch menu', (done) => {
    request(app)
      .get('/restaurants/voluptatum%20restaurant%209999000/menu/lunch')
      .then((response) => {
  ...
  test('should return correct data shape for dinner menu', (done) => {
    request(app)
      .get('/restaurants/voluptatum%20restaurant%209999000/menu/dinner')
      .then((response) => {
        expect(Array.isArray(response.body)).toBe(true);
        expect(response.body.length).toBe(20);
        done();
  ...
  test('should return correct data shape for dessert menu', (done) => {
    request(app)
      .get('/restaurants/voluptatum%20restaurant%209999000/menu/dessert')
      .then((response) => {
        expect(Array.isArray(response.body)).toBe(true);
  ...
  describe('returning only specified items based on their tags', () => {
    test('should return only gluten-free items', (done) => {
      request(app)
        .get('/restaurants/voluptatum%20restaurant%209999000/menu/lunch/gluten-free')
        .then((response) => {
          const tagObj = {};
    ...
    test('should return only vegan items', (done) => {
      request(app)
        .get('/restaurants/voluptatum%20restaurant%209999000/menu/lunch/vegan')
        .then((response) => {
          const tagObj = {};
          response.body.forEach((item) => {
            const tag = item.tags;
